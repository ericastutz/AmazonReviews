...
#   --data_prefix 'val' \
#   --out_dir ./out/55M_W0_1/full_eval_results \
#   --total_seqs 128 \
#   --batch_size 64 \
#   --max_seq_len 512 \
#   --summary_only True \
...
for b in tqdm(range(n_batches)):
    ...
    with torch.no_grad(), torch.autocast(device_type=device, dtype=torch.bfloat16):
        logits,attr_logits,_,_ = model.forward(x_pad,targets=y_pad) # (B,T,V) and (B,T,A)
    B,T,V = logits.shape
    ...
    n_minus_1_attr_logits = attr_logits[rows_to_use, n_minus_1_idx, :]
    n_minus_1_attr_probs = torch.softmax(n_minus_1_attr_logits, dim=-1)

    sampled_attr = torch.multinomial(n_minus_1_attr_probs, num_samples=1).squeeze(1)
    top1_attr = torch.argmax(n_minus_1_attr_probs, dim=-1)
    nll_attr = -torch.log(torch.gather(n_minus_1_attr_probs, 1, attrs[rows_to_use].unsqueeze(1)).clamp_min(1e-9)).squeeze(1)

    results['true_attr'].append(attrs[rows_to_use].tolist())
    results['sampled_attr'].append(sampled_attr.tolist())
    results['top1_attr'].append(top1_attr.tolist())
    results['cross_entropy_loss_attr'].append(nll_attr.tolist())
...
results['true_attr'] = [item for sublist in results['true_attr'] for item in sublist]
...
df_stratified_summary = (
    ...
    .agg(
        ...
        sampled_accuracy_attr=('sampled_attr',
            lambda x: np.mean(x.values == final_df.loc[x.index, 'true_attr'].values)
        ),
        top1_accuracy_attr=('top1_attr',
            lambda x: np.mean(x.values == final_df.loc[x.index, 'true_attr'].values)
        ),
