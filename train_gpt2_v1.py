...
def setup_model(model_cfg, model_name, out_dir, init_from, device):
    ...
    return model, optimizer, iter_global, best_val_loss, token_losses
...
def train(model,optimizer,tok,dl_train,dl_val,iter_global=0,best_val_loss=1000,token_losses=None,out_dir=None,stats=True,logging=True,generate_text=True):    

    logger = None
    if out_dir is not None:
        logger = Logger(out_dir,['iter','train_token_loss','train_val_loss','dt','tok_sec','epoch'])
    ...
    while iter_global <= model.config.max_iters:
        ...
        if iter_global % model.config.eval_interval == 0 and iter_global > 0:
            ...
            print(f"iter {iter_global}: loss {est_loss['train']['total']:.4f} | val loss {est_loss['val']['total']:.4f} | dt {dt*1000:.2f}ms | tok/sec {tokens_per_sec:.2f} | train epoch {dl_train.epoch}",flush=True)
            ...
            if logging and logger is not None:
                logger.log(iter_global,
                       est_loss['train']['total'].item(),
                       est_loss['val']['total'].item(),
...
if __name__ == "__main__":
    ...
    model, optimizer, iter_global, best_val_loss, token_losses = setup_model(model_cfg,model_name,out_dir,config_yaml['init_from'],device)

    train_token_file = os.path.join(data_dir,'train_token_ids.bin')
    val_token_file = os.path.join(data_dir,'val_token_ids.bin')

    dl_train = get_dataloader(dl_name,batch_size,max_seq_len,train_token_file,device)
    dl_val = get_dataloader(dl_name,batch_size,max_seq_len,val_token_file,device)
