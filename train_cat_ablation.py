...
def train(model,optimizer,tok,dl_train,dl_val,iter_global=0,best_val_loss=1000,token_losses=None,attr_losses=None,out_dir=None,stats=True,logging=True,generate_text=True):    
    ...
    while iter_global <= model.config.max_iters:
        ...
        token_loss = loss_accum.item() * model.config.token_weight - attr_loss_accum.item() * model.config.attr_weight
...
if __name__ == "__main__":
    ...
    model, optimizer, iter_global, best_val_loss, token_losses, attr_losses = setup_model(model_cfg,model_name,out_dir,config_yaml['init_from'],device)
