...
#   --data_prefix 'val' \
#   --out_dir ./out/reviews/55M_W0_1/full_eval_results \
#   --total_sequences 128 \
#   --batch_size 64 \
#   --max_seq_len 512 \
#   --summary_only True \
...
for b in tqdm(range(n_batches)):
    ...
    last_sampled = sampled_outcome[rows, last_idx] # (B,)
    last_top1 = top1_outcome[rows, last_idx] # (B,)
    last_ce = nll[rows, last_idx, attrs] # (B,)

    results['true_attr'].append(attrs.tolist())
    results['sampled_outcome'].append(last_sampled.tolist())
    results['top1_outcome'].append(last_top1.tolist())
...
results['sampled_outcome'] = [item for sublist in results['sampled_outcome'] for item in sublist]
results['top1_outcome'] = [item for sublist in results['top1_outcome'] for item in sublist]
...
final_df = pd.DataFrame({
    ...
    'sampled_outcome': results['sampled_outcome'],
    'top1_outcome': results['top1_outcome'],
