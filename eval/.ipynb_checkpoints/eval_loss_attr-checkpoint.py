...
# load data
dl_train = get_dataloader(dl_name,batch_size,max_seq_len,f'{data_dir}/train_token_ids.bin',f'{data_dir}/train_attr_ids.bin',device)
dl_val = get_dataloader(dl_name,batch_size,max_seq_len,f'{data_dir}/val_token_ids.bin',f'{data_dir}/val_attr_ids.bin',device)
...
if est_loss:
    results = {
        ...
        "val_total_loss": float(est_loss["val"]["token"]),
        "val_total_loss_var": float(est_loss["val"].get("token_var", float("nan"))),
        "val_total_loss": float(est_loss["val"]["attr"]),
        "val_total_loss_var": float(est_loss["val"].get("attr_var", float("nan")))
    }
